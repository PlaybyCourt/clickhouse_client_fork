...
RSpec.describe ClickHouse::Client::Redactor do
  let(:builder) { ClickHouse::Client::QueryBuilder.new(:test_table) }
  ...
  describe '.redact' do
    context 'when given simple conditions' do
      ...
      it 'redacts equality conditions correctly' do
        expected_redacted_sql = <<~SQL.squish.chomp.lines(chomp: true).join(' ')
    ...
    context 'when given IN conditions' do
      ...
      it 'redacts IN conditions correctly' do
        expected_redacted_sql = <<~SQL.squish.lines(chomp: true).join(' ')
            SELECT * FROM `test_table`
